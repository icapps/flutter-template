...
void main() {
  ...
  group('getRefreshToken', () {
    test('getRefreshToken with refresh-token', () async {
      when(secureStorage.getValue(key: 'ACCESS_REFRESH_TOKEN')).thenAnswer((_) async => 'refresh-token');

      final result = await sut.getRefreshToken();
      expect(result, 'refresh-token');
      verify(secureStorage.getValue(key: 'ACCESS_REFRESH_TOKEN')).calledOnce();
      verifyNoMoreInteractions(secureStorage);
    });
    test('getRefreshToken without refresh-token', () async {
      when(secureStorage.getValue(key: 'ACCESS_REFRESH_TOKEN')).thenAnswer((_) async => null);
  ...
  group('logout', () {
    test('getRefreshToken with refresh-token', () async {
      when(secureStorage.removeValue(key: 'ACCESS_REFRESH_TOKEN')).thenAnswer((_) => Future.value());
      when(secureStorage.removeValue(key: 'ACCESS_TOKEN')).thenAnswer((_) => Future.value());

      await sut.clear();
      verify(secureStorage.removeValue(key: 'ACCESS_REFRESH_TOKEN')).calledOnce();
  ...
  group('saveUserCredentials', () {
    test('getRefreshToken with refresh-token', () async {
      ...
      when(secureStorage.setValue(key: 'ACCESS_REFRESH_TOKEN', value: 'refresh-token')).thenAnswer((_) => Future.value());
      ...
      verify(secureStorage.setValue(key: 'ACCESS_REFRESH_TOKEN', value: 'refresh-token')).calledOnce();
      verifyNoMoreInteractions(secureStorage);
    });
  });
